...
from . import jdjsauth
from app import db
from ..models import jdUser
from ...utils import utils

@jdjsauth.route('/login', methods=['POST'])
def login():
    if request.json or 'username' not in request.json or 'password' not in request.json:
        user = jdUser.query.filter_by(username=request.json['username']).first()
        if user is not None :
...
@jdjsauth.route('/register', methods=['POST'])
def register():
    if request.json or 'username' not in request.json or 'password' not in request.json:
        user = jdUser(username=request.json['username'],
                    password=request.json['password'])
        olduser = jdUser.query.filter_by(username=request.json['username']).first()
